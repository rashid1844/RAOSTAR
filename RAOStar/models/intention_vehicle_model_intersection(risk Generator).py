...
class GeordiModel(object):                                                                                      #The POMDP model of the problem
    ...
    def __init__(self, vehicle_models=[], road=None, goal_state=(0,0), shift=10):
        ...
        self.action_map = {'side_A_left_left': 1000000000000000, 'side_A_left_forward': 100000000000000, 'side_A_right_forward': 10000000000000,
                            'side_A_right_right': 1000000000000, 'side_B_left_left': 100000000000, 'side_B_left_forward': 10000000000,
                            'side_B_right_forward': 1000000000, 'side_B_right_right': 100000000,
                            'side_C_left_left': 10000000, 'side_C_left_forward': 1000000, 'side_C_right_forward': 100000,
                            'side_C_right_right': 10000, 'side_D_left_left': 1000, 'side_D_left_forward': 100,
                            'side_D_right_forward': 10, 'side_D_right_right': 1, 'stop': 0}  # compute the sum to get binary number
                    # for Fast use 2 instead of 1

        self.action_map_binary = {'side_A_left_left': 32768, 'side_A_left_forward': 16384, 'side_A_right_forward': 8192,
                            ...
                            'side_D_right_forward': 2, 'side_D_right_right': 1, 'stop': 0}  # compute the sum to get binary number
    ...
    def compute_multi_risk(self, state, agent_names, debug=False):
        ...
        agent_action_list = []
        agent_action_fast = []
        agent_names_cp = agent_names.copy()  # a copy of agent_names to remove stationary agents
        agent_starting_line_bool = []  # bool if agent is at starting line (index==0)
        ...
        for agent in agent_names:
            ...
            if not agent_previous_action or 'stop' in agent_previous_action.name or self.prerequisite(state, agent):
                ...
                agent_starting_line_bool.append(agent_index == 0)
        ...
        for i,agent in enumerate(agent_names):
            if agent_starting_line_bool[i]:
                action_id += self.action_map_binary[state.get_state(agent).previous_action]

        inital_risk = precomuted_risk[action_id]
