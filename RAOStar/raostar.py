...
class RAOStar(object):
    ...
    def __init__(self, model, cc=0.0, cc_type='o', fixed_horizon=np.inf,                                                            #initilize cc "thefta, and defines cc type (Two types mentioned below)
        ...
        # bound at the root (constraints overall execution); "everywhere":                                                       #not sure the diffrenece
        ...
        self.fixed_horizon = fixed_horizon                                                                                #Horizion is infinite by default
        self.random_node_selection = random_node_selection                                                                #node selection options: random or best (this within a group of selected nodes
        self.end_search_on_likelihood = False

        self.debug("halting", self.halt_on_violation)

        # execution risk cap
        if self.cc_type.lower() in ['overall', 'o']:                                                          #cc type selection, overall means cc is predefined to 1.0 "er_cap"
        ...
        # choose the comparison function depending on the type of search                                       #defines problem as maximization or minimization
        ...
        # API functions from model (Model function aliases) #                                        #defines atrributes such as a,t,o,...
    ...
    def search(self, b0, time_limit=np.inf, iter_limit=np.inf):                                   #main function
        self.start_time = time.time()
        self.model.risk_time = 0.0
        ...
        while not self.search_termination(count, iter_limit, time_limit):
            ...
            expanded_nodes = self.expand_best_partial_solution()                                  #algorithm part1
            self.update_values_and_best_actions(expanded_nodes)                                   #algorithm part2
            # best actions aka policy
            # Updates the mapping of ancestors on the best policy graph and the
            # list of open nodes
            self.update_policy_open_nodes()                                              #updates leaf nodes of current best policy (one of the nodes of this update will be expanded in the next iteration)
        ...
        print('risk time is', self.model.risk_time)
        time_percent = (self.model.risk_time / (time.time() - self.start_time + 0.0001) ) * 100
        print('risk time percent is', time_percent)
        #print('risk algo one avg', self.model.risk_algo1 / self.model.risk_count)
        #print('risk algo two avg', self.model.risk_algo2 / self.model.risk_count)
        policy = self.extract_policy()

        return policy, self.graph

    def init_search(self, b0):                                                                                    #iinitilization of first node
    ...
    def set_new_node(self, node, depth, er_bound, prob, parent_likelihood):                  #creating new nodes (precisely, it's not actually instantiating, but fill values of newly instantiated node)
        ...
        node.depth = depth
        node.set_prob(prob)
        node.set_likelihood(prob * parent_likelihood)                                         #likelehood=parent likelihood * prob
        # Probability of violating constraints in a belief state. (never
        # change)
        if is_terminal_belief(b, self.term, self.terminal_prob):                              #terminal belif:
    ...
    def set_terminal_node(self, node):                                                                # once node reaches horizion or goal state
    ...
    def update_policy_open_nodes(self):
        ...
        self.likelihood_termination = self.nonterminal_leaf_sum + self.graph.root.exec_risk        #likelihood= non termin nodes likelihood sum + root er (auto terminates if enabled, which by defult not)
    ...
    def mark_deadend(self, node):                                                  #ignore
    ...
    def compute_exec_risk_bounds(self, parent_bound, parent_risk, child_list, prob_safe_list, is_terminal_node=False):                   #propegration risk bound (calc risk bound for other node)
    ...
    def build_ancestor_list(self, expanded_node):                                                               #find path from node to root node
    ...
    def extract_policy(self):                                                              #Output(result)
    ...
    def choose_node(self):                                                                    #select leave node (either random or best Q val)
    ...
    def choose_most_likely_leaf(self):                                              #similar to choose_node
        ...
        if len(self.opennodes) > 1:
            ...
            for node in self.opennodes:
                if node.likelihood > likelihood:                                          #finds the open node with highest liklyhood
                    most_likely = node
    ...
    def obtain_child_objs_and_probs(self, belief, T, O, r, act):                                                #get vals of each node
